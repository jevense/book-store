...
Elf.components.confirm=function(options){
    ...
    function init(){
        ...
        if(options.target){
        	Elf.appendTo(confirm,options.target);
        }
...
Elf.components.toast=function(options){
    ...
    //容器
    var toast=Elf.createElement("div","elf_toast");
    toast.body=Elf.createElement("div","elf_toast_body",toast);
    if(options.html){
        toast.body.innerHTML = options.html;
    }else{
        toast.bg=Elf.createElement("div","elf_toast_bg",toast.body);
        toast.body.text=Elf.createElement("div","elf_toast_text",toast.body);
        toast.body.text.innerHTML=options.text;
    }
    if(options.target){
    	Elf.appendTo(toast,options.target);
    }
    if(options.holdtime){
        setTimeout(function(){
    ...
    Elf.utils.extend(Elf.components,{
        loading:function(options,param){
            ...
            if(!Elf.components.loading.root){
                init(options,param);
                if(options.target){
                	Elf.appendTo(Elf.components.loading.root,options.target);
                }
            }
...
(function(){    
    function init(target,params){
        ...
        target.content=Elf.controls.createElement("div","elf-popup-content",target.content);
        target.background=Elf.controls.createElement("div","elf-popup-bg",target.content);

        target.closer=Elf.controls.createElement("div","elf-popup-closer",target.content);
        if(options.closeClass){
            Elf.utils.addClass(target.closer,options.closeClass);
        }        
        if(options.content){
        	if(typeof options.content=="string"){
        		var content=Elf.controls.createElement("div",target.content);
        		content.innerHTML=options.content;
        	}else{
        		Elf.controls.appendTo(options.content,target.content);
        	}
        }
        target.menus=Elf.controls.createElement("div","elf-popup-menus flex-box flex-direction-column",target.content);
