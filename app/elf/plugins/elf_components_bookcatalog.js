...
(function(){
    ...
    function setCurrentItem(target,item){
    ...
    function setCurrent(target,id){
    	var currentItem=target.nodeList[id];
    	if(currentItem){
    ...
    function getNext(target){
    	var next=target.currentItem.nextSibling;
    	var parent=Elf.utils.closest(target.currentItem.parentNode,"li");
    	var nextParent=parent.nextSibling;
    	if(next){
    		return next;
    	}else if(nextParent && nextParent.childNodeLs && nextParent.childNodeLs.children.length>0){
    		return nextParent.childNodeLs.children[0];
    	}else{
    		return {};
    	}
    }
    function getPrev(target){
    	var prev=target.currentItem.previousSibling;
    	var parent=Elf.utils.closest(target.currentItem.parentNode,"li");
    	var prevParent=parent.previousSibling;
    	if(prev){
    		return prev;
    	}else if(prevParent && prevParent.childNodeLs && prevParent.childNodeLs.children.length>0){
    		return prevParent.childNodeLs.children[prevParent.childNodeLs.children.length-1];
    	}else{
    		return {};
    	}
    }
    ...
    function createNode(target,obj,cover,pNode){
        ...
        var accordionNode=Elf.controls.createElement("div","elf-accordion-node ui-media",node);
        
        var accordionLeft=Elf.controls.createElement("div","ui-media-left",accordionNode);
        var accordionBody=Elf.controls.createElement("div","ui-media-body flex-column flex-justify-center",accordionNode);
        var accordionRight=Elf.controls.createElement("div","ui-media-right flex-column flex-justify-center",accordionNode);
        if(cover){
            Elf.controls.createElement("img",{src:cover},accordionLeft);
        }
        var handle=Elf.controls.createElement("i","elf-accordion-handle",accordionRight);
        text+=obj.pieceId?obj.pieceId+"&nbsp;&nbsp;":"";
        text+=obj.piece?obj.piece+"&nbsp;&nbsp;":"";
        if(obj.chapterId){
        	text+=obj.chapterId+"&nbsp;&nbsp;";
        }else if(obj.sectionId){
        	//if(obj.catalogShow=="false")
        	text+=JSON.stringify(obj.catalogShow)!="false"?obj.sectionId+"&nbsp;&nbsp;":"";
        }
        text+=obj.name||"";
        Elf.controls.createElement("a","elf-accordion-point",{innerHTML:text},accordionBody);
        node.ownData=obj;
        ...
        if(obj.sections && obj.sections.length>0){
        	...
        	Elf.utils.addClass(accordionLeft,'none');
        	Elf.utils.addClass(accordionRight,'none');
    ...
    function initView(target){
        ...
        if(opts.store){
            ...
            Elf.utils.each(opts.store,function(index,obj){
                createNode(target,obj,obj.icon);
            });
        }
        if(opts.currentId){
        	setCurrent(target,opts.currentId);
        }
    }
    ...
    Elf.components.bookcatalog.methods = {
    	...
    	getPrev:function(target){
    		return getPrev(target);
    	},
    	setCurrent:function(target,id){
